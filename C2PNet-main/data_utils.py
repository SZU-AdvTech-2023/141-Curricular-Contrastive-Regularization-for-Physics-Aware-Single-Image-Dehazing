...
#整个 LMDB_Image 类提供了一个方便的封装，用于将图像数据转换为可存储格式，并在需要时能够从该格式恢复图像。这在处理大规模图像数据集时尤其有用，因为LMDB可以提供快速的数据读取和写入性能。
class LMDB_Image:
    def __init__(self, haze, clear, n1, n2, n3, n4, n5, n6):
        #  存储图像的通道数，这对于后续重建图像很重要
        self.channels = haze.shape[2]
        # 存储图像的高度和宽度
        self.size = haze.shape[:2]
        # 将输入的numpy数组图像转换为字节串(bytes)，以便存储
...
class DatasetLMDB(data.Dataset):
    def __init__(self, db_path, size):
        ...
        #打开LMDB数据库环境，以只读模式且不加锁（适用于多进程读取）。
    ...
    def aug_data(self, *images):
        ...
        out.append(out[0])
        #如果设置了标准化选项，则对第一张图像（假设是主图像）应用标准化。
        if opt.norm:
    ...
    def __getitem__(self, index):
        ...
        # #将检索到的字节流反序列化成 LMDB_Image 对象，并获取所有图像。
        haze, clear, n1, n2, n3, n4, n5, n6 = IMAGE.get_image()
        #确保图像至少有指定的尺寸，如果没有，则随机选择其他图像。
        if isinstance(self.size, int):
...
OTS_train_loader_all = DataLoader(
    dataset=DatasetLMDB(os.path.join(path, '/home2/lsy/OTS/OTS.lmdb'), size=crop_size), batch_size=BS,
    shuffle=True, pin_memory=True)
